...
############################ Writing no growth function ############################
 
    def calculate_Ml_scipy_ng(self, l, k_fixed, k2_low, k2_high, z_low, z_high, step, stepsize = 0):
        if stepsize == 0:
            stepsize = (k2_high - k2_low) / float(step)
        else:
            step = int((k2_high - k2_low) / stepsize)

        filename = 'M_scipy_ng_'+str(int(l))+'('+str(k_fixed)+',['+str(k2_low)+','+str(k2_high)+']).'+str(step)+'steps.dat'
        x = [k2_low + float(i) * stepsize for i in range(0, step)]
        g = open(filename, 'w')
        g.close()
        
        for k2 in x:
            res = str(self.M_scipy_ng(l, k_fixed, k2, z_low, z_high))
            f = open(filename, 'a')
            f.write(str(k2)+" "+res+"\n")
            f.close()  

    def calculate_Ml_mp_ng(self, l, k_fixed, k2_low, k2_high, z_low, z_high, step, stepsize = 0):
        if stepsize == 0:
            stepsize = (k2_high - k2_low) / float(step)
        else:
            step = int((k2_high - k2_low) / stepsize)
            
        filename = 'M_mp_ng_'+str(int(l))+'('+str(k_fixed)+',['+str(k2_low)+','+str(k2_high)+']).'+str(step)+'steps.dat'
        stepsize = (k2_high - k2_low) / float(step)
        x = [k2_low + float(i) * stepsize for i in range(0, step)]
        g = open(filename, 'w')
        g.close()
        ...
        for k2 in x:
            res = str(self.M_mp_ng(l, k_fixed, k2, z_low, z_high))
            f = open(filename, 'a')
            f.write(str(k2)+" "+res+"\n")
            f.close() 
